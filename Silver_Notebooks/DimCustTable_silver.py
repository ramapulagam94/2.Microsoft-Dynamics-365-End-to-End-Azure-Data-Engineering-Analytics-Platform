...
df_custtable = spark.read.table("dev_catalog.bronze.custtable")
...
df_dimcusttable = df_custtable.filter(df_custtable.RecordId.isNotNull()
    ).select(
        df_custtable.CustomerId,
        F.when(df_custtable.LastProcessedChange_DateTime.isNull(), "1900-01-01").otherwise(df_custtable.LastProcessedChange_DateTime).cast("timestamp").alias("LastProcessedChange_DateTime"),
        F.from_utc_timestamp(df_custtable.DataLakeModified_DateTime,'CST').alias("DataLakeModified_DateTime"),
        F.trim(df_custtable.CustomerName).alias("CustomerName"),
        F.trim(df_custtable.Email).alias("Email"),
        F.trim(df_custtable.Phone).alias("Phone"),
        F.trim(df_custtable.Address).alias("Address"),
        F.trim(df_custtable.City).alias("City"),
        F.trim(df_custtable.State).alias("State"),
        F.trim(df_custtable.Country).alias("Country"),
        F.trim(df_custtable.Country).alias("ZipCode"),
        F.trim(df_custtable.Region).alias("Region"),
        F.from_utc_timestamp(df_custtable.SignupDate,'CST').alias("SignupDate"),
        df_custtable.RecordId.alias("CustRecordId")
    ).withColumn("UpdatedDateTime", F.current_timestamp()
    ).withColumn("PartyHashKey", F.xxhash64("CustRecordId")
    )
display(df_dimcusttable)

# COMMAND ----------

# DBTITLE 1,write to UC-Silver Schema
#reuse the function shared librabries
saveToDeltaToCatalog(df_dimcusttable, "dev_catalog", "silver", "dimcusttable") 